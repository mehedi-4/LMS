...
export default function PaymentSetup({ user }) {
  ...
  if (user?.isSet) return (
    <div className="max-w-2xl">
        <div className="bg-white rounded-lg shadow-lg p-8">
        <h2 className="text-3xl font-bold text-gray-900 mb-2">Payment Info</h2>
<div className="bg-blue-100 p-4 rounded-lg">
    <p className="text-gray-600">Account No: <span className="text-gray-800 font-medium">{user?.acc_no}</span></p>
    <p className="text-gray-600">Secret Key: <span className="text-gray-800 font-medium">{user?.secret_key}</span></p>
</div>
      </div>
      <div className="bg-white rounded-lg shadow-lg p-8">
        <h2 className="text-3xl font-bold text-gray-900 mb-2">Update Payment Info</h2>
        <p className="text-gray-600 mb-8">Configure your bank account details for receiving payments</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-indigo-600 text-white py-3 rounded-lg hover:bg-indigo-700 transition font-semibold disabled:bg-indigo-400 disabled:cursor-not-allowed"
          >
        ...
        <div className="mt-8 p-4 bg-indigo-50 border border-indigo-200 rounded-lg">
          ...
          <p className="text-sm text-gray-700">
            Your payment information is securely stored and will only be used for processing payments related to your courses.
          </p>
        </div>
      </div>
    </div>
